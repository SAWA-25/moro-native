...
export function extraPiecePrompt(p: {
    kind: ExtraPieceKind; charName: string; description: string; prompt?: string; userName: string; userBio?: string; options?: ExtraWorkshopOptions;
}): { sys: string; user: string } {
    const persona = personaLine(p.charName, p.description, p.userName, p.userBio);
