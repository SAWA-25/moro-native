...
- 若命令对应下列系统功能，除了生成角色的过渡消息（一两句即可）外，**必须**在回复末尾单独一行输出对应的指令标记，由系统弹出对应功能界面（标记不会显示给用户，缺了标记功能就不会真正发生）：
  ...
  · 转账 / 发钱 / 零花钱 → \`[[ACTION:TRANSFER:100]]\`（把 100 换成命令里的金额；若命令没写金额，按当下语境自定一个合理金额）
  · 发红包 / 普通红包 → \`[[ACTION:REDPACKET:100|祝福语]]\`（祝福语可省略）
  · 发口令红包 → \`[[ACTION:REDPACKET_PW:100|口令|祝福语]]\`（口令由你自定，并在正文里自然告诉或提示 ${userProfile.name}）
