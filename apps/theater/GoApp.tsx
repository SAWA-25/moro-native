...
import { ArrowClockwise, Flag, HandPalm, Play } from '@phosphor-icons/react';
import { useOS } from '../../context/OSContext';
import type { CharacterProfile, GoDifficultyMode, GoMoveEvent, Message, TheaterGoGame, TheaterGoInvitation } from '../../types';
...
import {
    GO_BOARD_SIZE,
    GO_DIFFICULTY_LABELS,
    GO_DIFFICULTY_MODE_LABELS,
    addGoDialogue,
    applyGoMove,
    applyGoPass,
    buildGoBoard,
    chooseGoMove,
    createGoGame,
    decideGoOpeningDifficulty,
    decideGoPerMoveDifficulty,
    fallbackGoDialogue,
    generateGoDialogue,
    getGoScore,
    goStoneForRole,
    normalizeGoGame,
    resignGoGame,
} from '../../utils/theaterGo';

export interface GoLaunchPayload {
    invitationId?: string;
    charId?: string;
    gameId?: string;
}

interface GoAppProps {
    onExit: () => void;
    launch?: GoLaunchPayload | null;
}

const modeOptions: Array<{ id: GoDifficultyMode; label: string; note: string }> = [
    { id: 'opening', label: '开局定档', note: '模型开局判断 TA 这一整盘的棋力。' },
...
const stoneLabel = (stone: 'black' | 'white') => stone === 'black' ? '黑' : '白';

const updateInviteCardMessage = async (invitation: TheaterGoInvitation, status: TheaterGoInvitation['status'], acceptedGameId?: string) => {
    try {
        const messages = await DB.getMessagesByCharId(invitation.charId, true);
        const target = [...messages].reverse().find((m: Message) => (m.metadata as any)?.goInvite?.invitationId === invitation.id);
        if (!target?.id) return;
...
const GoApp: React.FC<GoAppProps> = ({ onExit, launch }) => {
    const { characters, userProfile, apiConfig, auxApiConfig, updateCharacter, addToast } = useOS();
    const [games, setGames] = useState<TheaterGoGame[]>([]);
    const [invitations, setInvitations] = useState<TheaterGoInvitation[]>([]);
    const [selectedCharId, setSelectedCharId] = useState<string>('');
    const [difficultyMode, setDifficultyMode] = useState<GoDifficultyMode>('opening');
    const [game, setGame] = useState<TheaterGoGame | null>(null);
    ...
    const reload = useCallback(async () => {
        const [allGames, allInvites] = await Promise.all([
            DB.getAllTheaterGoGames().catch(() => []),
            DB.getAllTheaterGoInvitations().catch(() => []),
        ]);
        setGames(allGames.map(normalizeGoGame));
    ...
    useEffect(() => {
        ...
        const openLaunch = async () => {
            ...
            if (launch.gameId) {
                const found = await DB.getTheaterGoGame(launch.gameId);
                if (alive && found) setGame(normalizeGoGame(found));
            }
        };
        void openLaunch();
        return () => { alive = false; };
    ...
    const persistGame = useCallback(async (next: TheaterGoGame) => {
        const clean = normalizeGoGame(next);
        setGame(clean);
        await DB.saveTheaterGoGame(clean);
        setGames(prev => [clean, ...prev.filter(g => g.id !== clean.id)].sort((a, b) => b.lastActiveAt - a.lastActiveAt));
    }, []);

    const charTurn = useCallback(async (input: TheaterGoGame, seedEvents: GoMoveEvent[] = []) => {
        const clean = normalizeGoGame(input);
        ...
        setBusy(true);
        let thinking = addGoDialogue(clean, 'thinking', fallbackGoDialogue('thinking', char.name), 'char', undefined);
        await persistGame(thinking);

        let level = thinking.difficultyLevel;
        if (thinking.difficultyMode === 'per_move') {
            const assessed = await decideGoPerMoveDifficulty(char, userProfile, api, thinking, seedEvents);
            level = assessed.difficultyLevel;
            thinking = { ...thinking, difficultyLevel: level };
        }

        const picked = chooseGoMove(thinking, level);
        const applied = 'pass' in picked && picked.pass
            ? applyGoPass(thinking, 'char')
            : applyGoMove(thinking, picked.row, picked.col, 'char');
        if (!applied.ok) {
            const passed = applyGoPass(thinking, 'char');
            if (!passed.ok) {
                setBusy(false);
                return thinking;
            }
            const text = await generateGoDialogue(char, userProfile, api, passed.game, passed.events, passed.move);
            const withLine = addGoDialogue(passed.game, passed.events[0] || 'normal', text, 'char', passed.move.no);
            await persistGame(withLine);
            setBusy(false);
            return withLine;
        }

        const text = await generateGoDialogue(char, userProfile, api, applied.game, applied.events, applied.move);
        const withLine = addGoDialogue(applied.game, applied.events[0] || 'normal', text, 'char', applied.move.no);
        await persistGame(withLine);
        setBusy(false);
        return withLine;
    }, [api, characters, persistGame, userProfile]);

    const startGame = useCallback(async (char: CharacterProfile, mode: GoDifficultyMode, opts?: { invitation?: TheaterGoInvitation; charStarts?: boolean }) => {
        setBusy(true);
        try {
            const assessed = await decideGoOpeningDifficulty(char, userProfile, api, mode, !!opts?.invitation);
            let next = createGoGame(userName, char, {
                difficultyMode: mode,
            ...
            const opening = assessed.reason
                ? `我按自己的感觉来下，${GO_DIFFICULTY_LABELS[assessed.difficultyLevel]}档，理由先不告诉你。`
                : fallbackGoDialogue('invite', char.name);
            next = addGoDialogue(next, 'invite', opts?.invitation?.message || opening, 'char');
            await persistGame(next);
            if (opts?.invitation) {
                const accepted = { ...opts.invitation, status: 'accepted' as const, acceptedGameId: next.id, updatedAt: Date.now() };
                await DB.saveTheaterGoInvitation(accepted);
                await updateInviteCardMessage(accepted, 'accepted', next.id);
                setInvitations(prev => prev.map(i => i.id === accepted.id ? accepted : i));
            }
            addToast(`围棋开局：${charDisplay(char)} · ${GO_DIFFICULTY_LABELS[assessed.difficultyLevel]}`, 'success');
            if (next.currentTurn === 'char') await charTurn(next);
        } catch {
            addToast('围棋开局失败了，稍后再试一次', 'error');
        } finally {
            setBusy(false);
        }
    ...
    const onUserMove = useCallback(async (row: number, col: number) => {
        ...
        if (!char) return;
        setBusy(true);
        const applied = applyGoMove(game, row, col, 'user');
        if (!applied.ok) {
            const warned = addGoDialogue(game, 'illegal', applied.reason, 'system');
    ...
    const onUserPass = useCallback(async () => {
        if (!game || busy || game.status === 'ended' || game.currentTurn !== 'user') return;
        const char = characters.find(c => c.id === game.charId);
        if (!char) return;
        setBusy(true);
        const passed = applyGoPass(game, 'user');
        if (!passed.ok) {
            const warned = addGoDialogue(game, 'illegal', passed.reason, 'system');
            await persistGame(warned);
            setBusy(false);
            return;
        }
        const text = await generateGoDialogue(char, userProfile, api, passed.game, passed.events, passed.move);
        const withLine = addGoDialogue(passed.game, passed.events[0] || 'pass', text, 'char', passed.move.no);
        await persistGame(withLine);
        if (withLine.status !== 'ended') {
            await charTurn(withLine, passed.events);
        } else {
            setBusy(false);
        }
    }, [api, busy, charTurn, characters, game, persistGame, userProfile]);
    ...
    const resign = useCallback(async () => {
        ...
        const ended = addGoDialogue(resignGoGame(game, 'user'), 'win', '我认输。这盘算你赢。', 'system');
        await persistGame(ended);
    }, [game, persistGame]);

    const declineInvite = useCallback(async (inv: TheaterGoInvitation) => {
        const next = { ...inv, status: 'declined' as const, updatedAt: Date.now() };
        await DB.saveTheaterGoInvitation(next);
    ...
    const toggleProactive = useCallback((char: CharacterProfile) => {
        updateCharacter(char.id, {
            convoSettings: {
                ...(char.convoSettings || {}),
                proactiveGoInvite: !char.convoSettings?.proactiveGoInvite,
            },
        });
    }, [updateCharacter]);
    ...
    return (
        <PaperShell>
            <ScrapHeader title="围棋" en="GO" onBack={onExit} />
            <ScrapScroll className="px-4 pb-10">
                <div className="space-y-5">
                    <PaperCard className="px-5 py-5 overflow-hidden">
                        <div className="text-[9px] tracking-[0.32em] mb-1" style={{ fontFamily: 'var(--font-label)', color: INK_SOFT }}>ACT XII</div>
                        <div className="text-[30px] font-black" style={{ color: INK }}>拾贰 · 围棋</div>
                        <p className="mt-2 text-[12.5px] leading-relaxed" style={{ color: '#5b554a' }}>
                            19 路休闲围棋：吃子、禁自杀、简单 ko，可停着；双方连续停着后用面积法估算胜负。TA 的棋力由模型按完整设定判断，落子仍由本地规则兜底。
                        </p>
                    </PaperCard>
                                        ...
                                        <PaperCard key={inv.id} className={`px-4 py-4 ${highlighted ? 'ring-2 ring-[#1f1d1a]' : ''}`}>
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div className="min-w-0 flex-1">
                                                    <div className="text-[14px] font-black" style={{ color: INK }}>{inv.charName} 约你手谈一局</div>
                                                    <div className="mt-1 text-[12px] leading-relaxed" style={{ color: '#5b554a' }}>{inv.message || '要不要手谈一局？'}</div>
                                                    <div className="mt-1 text-[10px]" style={{ color: INK_SOFT }}>{GO_DIFFICULTY_MODE_LABELS[inv.difficultyMode || difficultyMode]} · {fmtTime(inv.createdAt)}</div>
                                                </div>
                                            </div>
                            ...
                            {selectedChar && (
                                <div className="mt-4 flex items-center justify-between gap-3 rounded-[14px] px-3 py-3" style={{ background: '#fffdfa', border: '1px solid rgba(31,29,26,0.08)' }}>
                                    <div className="min-w-0">
                                        <div className="text-[12px] font-black" style={{ color: INK }}>允许 TA 主动约围棋</div>
                                        <div className="text-[10px] mt-0.5" style={{ color: INK_SOFT }}>旧角色默认关闭；打开后聊天主动消息可生成围棋邀请卡。</div>
                                    </div>
                                    <button onClick={() => toggleProactive(selectedChar)} className="relative h-7 w-12 shrink-0 rounded-full transition" style={{ background: selectedChar.convoSettings?.proactiveGoInvite ? '#1f1d1a' : '#e6e1d8' }}>
                                        <span className="absolute top-1 h-5 w-5 rounded-full bg-white transition-all" style={{ left: selectedChar.convoSettings?.proactiveGoInvite ? 24 : 4 }} />
                                    </button>
                                </div>
                            )}
                        ...
                        <section>
                            ...
                            <div className="mt-3 space-y-3">
                                {activeGames.slice(0, 5).map(g => (
                                    <PaperCard key={g.id} onClick={() => setGame(normalizeGoGame(g))} className="px-4 py-3 flex items-center gap-3">
                                        <Stamp size={38}>围</Stamp>
                                        <div className="min-w-0 flex-1">
                                            <div className="text-[13px] font-black truncate" style={{ color: INK }}>{g.charName}</div>
                                            <div className="text-[10.5px] mt-0.5 truncate" style={{ color: INK_SOFT }}>{g.moves.length} 手 · {GO_DIFFICULTY_LABELS[g.difficultyLevel]} · {g.currentTurn === 'user' ? '轮到你' : '轮到 TA'}</div>
                                        </div>
...
const Avatar: React.FC<{ char?: CharacterProfile | null }> = ({ char }) => (
    <div className="h-11 w-11 shrink-0 overflow-hidden rounded-full" style={{ background: '#e8e1d5', border: '1px solid rgba(31,29,26,0.12)' }}>
        {char?.avatar ? <img src={char.convoSettings?.charAvatarOverride || char.avatar} className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center font-black">围</div>}
    </div>
);

const starPoints = new Set(['3,3', '3,9', '3,15', '9,3', '9,9', '9,15', '15,3', '15,9', '15,15']);
...
const PlaySurface: React.FC<{
    ...
    onResign: () => void;
    onRestart: () => void;
}> = ({ game, char, userName, busy, onMove, onPass, onResign, onRestart }) => {
    const board = useMemo(() => buildGoBoard(game.moves, game.boardSize || GO_BOARD_SIZE), [game]);
    const last = [...game.moves].reverse().find(m => !m.pass);
    const userStone = goStoneForRole(game, 'user');
    const charStone = goStoneForRole(game, 'char');
    const score = game.score || getGoScore(game);
        ...
        <div className="space-y-4">
            <PaperCard className="px-4 py-4">
                <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="text-[10px] mt-0.5" style={{ color: INK_SOFT }}>
                            {GO_DIFFICULTY_MODE_LABELS[game.difficultyMode]} · {GO_DIFFICULTY_LABELS[game.difficultyLevel]} · TA 执{stoneLabel(charStone)}，你执{stoneLabel(userStone)}
                        </div>
                    </div>
                ...
                <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                    <MiniScore label="黑提子" value={`${game.captures?.black || 0}`} />
                    <MiniScore label="白提子" value={`${game.captures?.white || 0}`} />
                    <MiniScore label="估目" value={`黑${score.black.toFixed(1)} / 白${score.white.toFixed(1)}`} />
                </div>
            </PaperCard>

            <PaperCard className="p-3">
                <div className="mx-auto aspect-square w-full max-w-[min(92vw,600px)] rounded-[12px] p-[8px]" style={{ background: '#d8c39b', border: '1px solid rgba(31,29,26,0.20)', boxShadow: 'inset 0 0 0 1px rgba(255,255,255,0.25)' }}>
                    <div
                        className="grid h-full w-full"
                        style={{
                            gridTemplateColumns: `repeat(${game.boardSize || GO_BOARD_SIZE}, minmax(0, 1fr))`,
                            gridTemplateRows: `repeat(${game.boardSize || GO_BOARD_SIZE}, minmax(0, 1fr))`,
                            ...
                            backgroundSize: `${100 / ((game.boardSize || GO_BOARD_SIZE) - 1)}% ${100 / ((game.boardSize || GO_BOARD_SIZE) - 1)}%`,
                            backgroundPosition: 'center',
                        }}
                    >
                        {board.flatMap((row, r) => row.map((stone, c) => {
                            const isLast = last?.row === r && last?.col === c;
                            const isStar = starPoints.has(`${r},${c}`);
                            return (
                                <button
                                    key={`${r}-${c}`}
                                ...
                                >
                                    {!stone && isStar && <span className="h-[18%] w-[18%] rounded-full" style={{ background: 'rgba(31,29,26,0.44)' }} />}
                                    {stone && (
                                        <span
                                            className="block h-[78%] w-[78%] rounded-full"
                                            style={{
                                                ...
                                                outline: isLast ? '2px solid rgba(232,95,122,0.9)' : undefined,
                                            }}
                                        />
                                    )}
                                </button>
            ...
            <div className="grid grid-cols-3 gap-2">
                <ScrapButton variant="ghost" className="h-10 text-[12px]" icon={<HandPalm size={15} />} disabled={game.status === 'ended' || busy || game.currentTurn !== 'user'} onClick={onPass}>停着</ScrapButton>
                ...
                <ScrapButton variant="paper" className="h-10 text-[12px]" icon={<ArrowClockwise size={15} />} disabled={busy} onClick={onRestart}>重开</ScrapButton>
            </div>
...
const MiniScore: React.FC<{ label: string; value: string }> = ({ label, value }) => (
    <div className="rounded-[12px] px-2 py-2" style={{ background: 'rgba(255,253,247,0.72)', border: '1px solid rgba(176,170,158,0.48)' }}>
        <div className="text-[12px] font-black truncate" style={{ color: INK }}>{value}</div>
        <div className="text-[9px] mt-0.5" style={{ color: INK_SOFT }}>{label}</div>
    </div>
);
