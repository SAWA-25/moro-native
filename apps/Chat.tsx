...
const screenPeekDurationText = (minutes?: number) => {
...
const buildUserPhonePeekNarrative = (snapshot: ScreenPeekDeviceSnapshot, charName: string) => {
    if (snapshot.source === 'android_screen_capture') {
        return `${charName} 离开自己的手机，以系统悬浮窗贴到你的真实手机旁。你授权后，Moro 会用 Android 录屏给 TA 一张低清实时画面，让 TA 评论你此刻的真实桌面或其它 App；你可以随时停掉录屏。`;
    }
    if (snapshot.source === 'screen_capture_permission_required') {
        return `${charName} 已经站到你的手机边上，但还需要 Android 录屏授权；如果要离开 Moro 后也看到评论，还需要打开系统悬浮窗权限。未授权前不会生成假评论。`;
    }
    if (snapshot.source === 'android_usage_stats') {
        return `${charName} 暂时只能看到 Android 授权的使用情况，真正的窥屏评论需要录屏授权后才会开始。`;
    }
    if (snapshot.source === 'permission_required') {
        return `${charName} 还没有拿到真实手机相关授权。未授权前不会生成假评论。`;
    }
    return snapshot.unavailableReason || '当前平台暂不支持读取真实手机使用情况，TA 不会假装看见你的屏幕。';
};

const buildUserPhonePeekScreen = (
    snapshot: ScreenPeekDeviceSnapshot,
    char: CharacterProfile,
    now: number,
): NonNullable<ScreenPeekCard['screen']> => {
    type ObservedApp = NonNullable<ScreenPeekDeviceSnapshot['currentForegroundApp']>;
    const observed = pickObservedUserPhoneApp(snapshot);
    const seen = new Set<string>();
    const apps = ([observed, ...(snapshot.appUsage || [])] as Array<ObservedApp | undefined>)
        .filter((app): app is ObservedApp => {
            if (!app) return false;
            const key = app.packageName || app.appId || app.appName;
            if (!key || seen.has(String(key))) return false;
            seen.add(String(key));
            return true;
        })
        .slice(0, 8);
    const summaryLines = summarizeScreenPeekDeviceSnapshot(snapshot)
        .split('\n')
        .map(line => line.trim())
        .filter(Boolean);
    const rows = snapshot.source === 'android_screen_capture' || snapshot.source === 'android_usage_stats'
        ? ([
            ...(snapshot.screenFrame ? [{
                id: 'user-phone-frame',
                title: '授权录屏画面',
                body: `已拿到一帧 ${snapshot.screenFrame.width || '?'}x${snapshot.screenFrame.height || '?'} 的实时画面，TA 会按这张画面评论。`,
                meta: screenPeekClock(snapshot.screenFrame.capturedAt || now),
                badge: '录屏中',
            }] : []),
            ...(apps.length ? apps.map((app, index) => {
            const duration = screenPeekDurationText(app.durationMinutes);
            const meta = app.lastTimeUsed ? screenPeekClock(app.lastTimeUsed) : duration;
            return {
                id: `user-phone-app-${index}`,
                title: app.appName || '未知 App',
                subtitle: app.packageName,
                body: clipForPreview([
                    app.isMoro ? 'Moro 当前在前台。' : (index === 0 ? 'TA 最近能看到你停在这里。' : '今日使用痕迹。'),
                    duration ? `今日停留 ${duration}。` : '',
                    app.note || '',
                ].filter(Boolean).join(' '), 92),
                meta: meta || (index === 0 ? '当前/最近' : '今日'),
                badge: index === 0 ? '正在看' : undefined,
            };
            }) : summaryLines.slice(0, 5).map((line, index) => ({
            id: `user-phone-summary-${index}`,
            title: index === 0 ? '真实手机状态' : `状态 ${index + 1}`,
            body: line,
            meta: screenPeekClock(now),
            }))),
        ].slice(0, 8))
        : [{
            id: 'user-phone-unavailable',
            title: snapshot.source === 'screen_capture_permission_required' || snapshot.source === 'permission_required' ? '需要授权' : '当前不可用',
            body: snapshot.source === 'screen_capture_permission_required'
                ? '请先允许 Android 录屏；若要回到真实桌面后仍看到评论，还要打开悬浮窗权限。'
                : (snapshot.unavailableReason || 'Web/PWA/iOS 暂不支持真实录屏窥屏。'),
            meta: snapshot.source === 'screen_capture_permission_required' ? '录屏授权' : '不伪造窥屏',
        }];
    const observedName = observed?.appName || (snapshot.source === 'screen_capture_permission_required' || snapshot.source === 'permission_required' ? '等待授权' : '真实手机');
    const batteryLevel = typeof snapshot.batteryLevel === 'number' && snapshot.batteryLevel >= 0 ? snapshot.batteryLevel : 72;
    return {
        appKind: 'app',
        appName: observedName,
        title: snapshot.source === 'android_screen_capture'
            ? '真实手机录屏'
            : snapshot.source === 'android_usage_stats'
                ? observedName
                : (snapshot.source === 'screen_capture_permission_required' || snapshot.source === 'permission_required' ? '真实手机授权' : '真实手机不可用'),
        subtitle: `${char.name} 看你的手机`,
        action: buildUserPhonePeekNarrative(snapshot, char.name),
        timeText: screenPeekClock(now),
        batteryLevel,
        avatar: char.avatar,
        hero: snapshot.screenFrame ? {
            title: '实时录屏快照',
            subtitle: '低清保存一帧用于这张窥屏卡；后续评论只写文字回卡片。',
            imageUrl: snapshot.screenFrame.dataUrl,
        } : undefined,
        rows,
    };
};
...
${parallelReplyPromptBody({
    ...
    const drainAutoReplyQueue = useCallback(async () => {
        ...
        if (!queueChar.convoSettings?.autoReplyEachUserMessage || queueChar.blacklisted || queueChar.charBlock?.active) {
            autoReplyQueueRef.current = [];
            return;
        }
        ...
        try {
            while (autoReplyQueueRef.current.length > 0) {
                ...
                if (
                    ...
                    liveChar.blacklisted ||
                    liveChar.charBlock?.active
                ) {
                    autoReplyQueueRef.current = [];
                    break;
                }
    ...
    useEffect(() => {
        if (!char?.convoSettings?.autoReplyEachUserMessage || char.blacklisted || char.charBlock?.active) {
            clearAutoReplyQueue();
        }
    ...
    const triggerLiveSendReply = useCallback((fromQueue = false) => {
        if (!char || !liveChatEnabled || char.blacklisted || char.charBlock?.active) return;
        if (isTyping) {
    ...
    const triggerLiveDraftReply = useCallback((draftText: string) => {
        if (!char || char.blacklisted || char.charBlock?.active || isTyping) return;
        const now = Date.now();
    ...
    const scheduleLiveDraftCheck = useCallback((draftText: string) => {
        ...
        if (!char || char.blacklisted || char.charBlock?.active) return;
    ...
    const runLivePrivateInterjects = async (sourceChar: CharacterProfile, userText: string) => {
        ...
        const replyApi = resolveAuxApi(auxApiConfig, apiConfig);
        if (!replyApi.baseUrl || !replyApi.apiKey || !replyApi.model) return;
...
${livePrivateInterjectPromptBody({
                ...
                }, 1, 45000, makeApiUsageMeta('chat.livePrivateInterject', {
                    charId: target.id,
                    charName: target.name,
                    apiRole: isAuxApiOn(auxApiConfig) ? 'aux' : 'main',
                }));
...
${privateCallDecisionPromptBody({
    ...
    const handleScreenPeek = async () => {
        ...
        try {
            ...
            const captureSnapshot = await startRealPhoneScreenCapture({ title: `${displayName} 正在看你的手机` });
            const usageSnapshot = await getRealPhoneUsageSnapshot({
                rangeStart: now - 8 * 60 * 60 * 1000,
                rangeEnd: now,
                limit: 12,
            }).catch(() => null);
            const deviceSnapshot: ScreenPeekDeviceSnapshot = usageSnapshot?.source === 'android_usage_stats'
                ? {
                    ...usageSnapshot,
                    ...captureSnapshot,
                    source: captureSnapshot.source,
                    unavailableReason: captureSnapshot.unavailableReason || usageSnapshot.unavailableReason,
                ...
                : captureSnapshot;
            const screen = buildUserPhonePeekScreen(deviceSnapshot, char, now);
            const card: ScreenPeekCard = {
                id: `screen-peek-user-phone-${char.id}-${now}`,
                charId: char.id,
                charName: displayName,
                generatedAt: now,
                title: `${displayName} 正在看你的真实手机`,
                narrative: buildUserPhonePeekNarrative(deviceSnapshot, displayName),
                        ...
                        return (
                            <div className="flex items-center justify-between gap-2 px-4 py-2.5 bg-amber-50 border-b border-amber-200 text-xs">
                                ...
                                <button
                                    ...
                                    className="px-2.5 py-1 bg-amber-500 text-white rounded-full text-[11px] font-bold active:scale-95 shrink-0"
                                >
                                    查看申请
                                </button>
                            </div>
